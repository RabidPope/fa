...
--**
--** UI-only. Queues a real unit ID via UNITCOMMAND_BuildFactory, the same command
--** the construct grid uses. The sim never sees extra payload.
--******************************************************************************************************
...
local function SelectedCybranGates()
    ...
    local gates = EntityCategoryFilterDown(CybranGatewayCategory, selection)
    return gates
end
...
--- Choices for one slot, including empty. Each choice is the full prereq chain.
...
--- Flatten selected slot choices into one enhancement list.
--- @return string[]
...
function CreateDialog()
    ...
    local hint = UIUtil.CreateText(dialog, 'Pick one option per slot, then queue. Hidden combo IDs stay off the construct grid.', 11, UIUtil.bodyFont)
